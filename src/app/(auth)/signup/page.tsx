...
import { Mail, ArrowRight, ArrowLeft, Loader2, User } from "lucide-react";
import { Button, Input } from "@/components/ui";
import { BrandLogo } from "@/components/ui/BrandLogo";
import { useAuth } from "@/components/providers";
import { signInWithGoogle } from "@/lib/auth"; // keep if needed, but useAuth already provides it
...
// Main signup form component that uses useSearchParams
function SignupForm() {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        // Redirect to the new flow page passing state via query params
        // You would typically use a shared state or url params.
        router.push(`/auth/email-login?email=${encodeURIComponent(email)}&flow=${res.isNewUser ? 'SIGNUP_DETAILS' : 'LOGIN_OTP'}&name=${encodeURIComponent(name)}`);
      } else {
      ...
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-text-secondary">
            Join Aiel Enterprises Toy Paradise
          </p>
        </div>
